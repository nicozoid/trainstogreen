#!/usr/bin/env node
// Rekeys data/station-notes.json from coordKey to station ID. Same
// approach as scripts/migrate-station-months.mjs — see that file's
// comments for the strategy. Idempotent; running it twice on already-
...
const notesPath = path.join(ROOT, "data/station-notes.json")
const notes = JSON.parse(fs.readFileSync(notesPath, "utf8"))

const migrated = {}
const orphans = []
for (const [oldKey, value] of Object.entries(notes)) {
  // Already-migrated entries (no comma in key) pass through.
  if (!oldKey.includes(",")) {
...
fs.writeFileSync(notesPath, JSON.stringify(sorted, null, 2) + "\n")
