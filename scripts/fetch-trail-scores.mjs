// One-off script to enrich stations.json with a trail density score.
// Queries Overpass API (free, no key needed) to count OSM footpaths,
// bridleways, and tracks within 5km of each station.
//
// Run with: node scripts/fetch-trail-scores.mjs
//
...
// Multiple public Overpass instances — we rotate through them to avoid rate limits
...
// Count OSM ways tagged as walkable paths within the given radius.
// Retries up to 3 times on failure. Returns null if all attempts fail.
async function getTrailCount(lat, lng) {
  // [out:json]       — return JSON, not XML
  // [timeout:60]     — give Overpass up to 60s per query
  // way(around:...)  — ways within radius of lat/lng
  // [highway~"..."]  — only ways tagged as footway, path, bridleway, or track
  // out count        — return just the count, not full geometry (much faster)
  const query = `
    [out:json][timeout:60];
    way(around:${RADIUS_METRES},${lat},${lng})[highway~"^(footway|path|bridleway|track)$"];
    out count;
  `
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (!res.ok) {
        ...
        await sleep(5000 * attempt) // back off longer on each retry
        continue
      }
...
async function main() {
  ...
  for (const feature of data.features) {
    ...
    // Skip stations that already have a trail score (allows resuming)
    if (feature.properties.trailScore != null) {
      skipped++
      continue
    }

    const [lng, lat] = feature.geometry.coordinates
    const count = await getTrailCount(lat, lng)
    feature.properties.trailScore = count

    processed++
    const pct = Math.round(((processed + skipped) / total) * 100)
    console.log(`[${pct}%] ${name}: ${count ?? "FAILED"} trails`)
