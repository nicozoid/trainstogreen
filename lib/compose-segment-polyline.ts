// Origin: ported from branch claude/admiring-goldberg-2908b7 (commit 578cf56).
// Adapted into main without that branch's surrounding pipeline scripts or
// routing-diff regenerator changes — only the runtime composer + its
// segment-library data file. The high-level layer (buildComposeContext /
// composeFullJourneyPolyline) declares its own input shapes locally, so it
// compiles standalone but expects callers to provide data in those shapes.
//
